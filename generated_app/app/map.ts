...
import { normalizeToGeometryKey, HOBOKEN_STREET_LAT, HOBOKEN_STREET_LAT_TOLERANCE, HOBOKEN_STREET_LAT_PER_STREET_TOL, filterWaysByStreetLat, flattenWaysToArcPath, clipWaysToArcRange } from "../shared/cleaning-geometry";
...
function buildWayPolylines(
  ...
  const weight = useLaneSplit ? Math.max(2, laneWidthPx) : Math.max(3, laneWidthPx * 2);
...
// flattenWaysToArcPath and clipWaysToArcRange live in shared/cleaning-geometry.ts (CF-20).
// Re-exported here for test compatibility (address-arc.test.ts imports from app/map).
export { flattenWaysToArcPath, clipWaysToArcRange } from "../shared/cleaning-geometry";
