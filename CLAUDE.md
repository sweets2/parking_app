...
The OSM bounding box used to build `road-geometry.json` is slightly larger than Hoboken, so generic street names like "3RD ST", "4TH ST", "5TH ST" pick up Jersey City Heights ways at lat ~40.753 — about 0.013° north of Hoboken's actual 3rd St (~40.740). `renderSnowEmergencyRoutes` guards against this with a centroid-lat filter: `HOBOKEN_STREET_LAT` maps each numbered street to its expected Hoboken latitude, and any way whose centroid deviates more than `HOBOKEN_STREET_LAT_TOLERANCE` (0.008°) is skipped. Do not remove this guard — without it, phantom routes appear visually in Jersey City. The expected latitudes were calibrated against real OSM data: `"9TH ST": 40.748` (not 40.750 — a JC Heights stub at centLat 40.757 would otherwise slip through) and `"13TH ST": 40.753` (not 40.756 — a JC segment at centLat 40.761 spanning Palisade Ave to Grand Ave would otherwise slip through).

**CF-19 — Street-cleaning geometry contamination fix (filterWaysByStreetLat)**
`HOBOKEN_STREET_LAT` was expanded from 5 to all 16 numbered streets (calibrated from road-geometry.json centroid audit). A new `filterWaysByStreetLat(ways, streetKey)` helper applies this filter before any boundary or clip computation. It is applied in: `parseLocationBounds` (so `getBoundaryCoord` uses clean geometry), `renderViolationHighlights` (final guard after clip), and `resolveCheckCurbWays` (check-mode stripes). For 11TH, 12TH, and 13TH ST the JC centroid gap is ≤ 0.008°; `HOBOKEN_STREET_LAT_PER_STREET_TOL` provides tighter overrides: 11TH ST → 0.007, 12TH ST → 0.006, 13TH ST → 0.007. Calibrated expected lats: 1ST=40.738, 2ND=40.739, 3RD=40.740, 4TH=40.741, 5TH=40.742, 6TH=40.744, 7TH=40.746, 8TH=40.747, 9TH=40.748, 10TH=40.749, 11TH=40.750, 12TH=40.752, 13TH=40.753, 14TH=40.754, 15TH=40.755, 16TH=40.757. A real-data audit in the test suite confirms no numbered-street segment renders in a JC cluster; non-numbered leaks (one point at lat≈40.760 at time of writing) are WARN-only and do not block CI.

**F-37 — Per-route coordinate clipping (`minLon`/`maxLon`/`minLat`/`maxLat`)**
`SnowRoute` carries optional `minLon`, `maxLon`, `minLat`, `maxLat` fields that clip each route's rendered geometry to its designated cross-street extents. `clipWaysByBounds()` in `app/map.ts` applies these after the centroid-lat filter and before `mergeWays()`. Use `minLon`/`maxLon` for east-west streets (numbered streets — cross streets run north-south at constant longitude) and `minLat`/`maxLat` for north-south streets (e.g., `WILLOW AVE` uses `maxLat: 40.752` to stop at 11th St). Determine clip values by looking up the cross-street's ways in `data/road-geometry.json` and reading the longitude/latitude at the intersection. Without these fields, the full OSM geometry for the street renders — which often extends well past the designated snow route endpoints.

**Tow toggle visibility — use explicit layer management, not CSS pane targeting**
`setTowSignsVisible` controls both pin markers (`_signLayers`) and polylines (`_segmentLayers`) by calling `layer.addTo(_map)` / `layer.remove()` directly — the same pattern used by garage and upcoming-sign markers. CSS pane targeting (hiding `.leaflet-towSignPane-pane` via a `tow-signs-hidden` class on `#map`) was tried and abandoned because it proved unreliable in the browser. `renderSignPins` gates `marker.addTo(_map)` behind `if (_towSignsVisible)` so newly-rendered markers respect the toggle state. Do not revert to a CSS approach or extend it to new marker types.
